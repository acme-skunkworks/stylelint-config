...
- Rename `name` in `package.json` (the placeholder is `@rheged-studio/npm-package-template`) and update `description`/`keywords`/`repository`/`homepage`/`bugs`.
...
- **Lint configs are inherited as-is.** `eslint.config.ts` (extends `@rheged-studio/eslint-config`) and `.markdownlint-cli2.jsonc` (extends `@rheged-studio/markdownlint-config`) carry the org defaults; a generated repo needs no edit. Extend `eslint.config.ts` only to pull in the opt-in presets the package needs (`testing`, `frameworkRouting`, `astro`, `sanity`, `storybook`, `tableComponents` — all re-exported from the preset).
